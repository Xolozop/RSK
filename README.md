...
[Информация о сборке проекта](#build)
...
Алгоритм использует структуру Cell для хранения информации о ребрах ячеек и наличии точек. Инициализируются таблицы P и Q, определяется размер перестановки n, создаются вектора current и next для текущей и следующей строки ячеек. Внешний цикл проходит по строкам, внутренний — по столбцам. Если ячейка содержит точку, устанавливаются значения правого и верхнего ребер. Если нижнее и левое ребра ячейки равны друг другу и не равны нулю, обновляются правое и верхнее ребра. Иначе, правое ребро принимает значение левого, а верхнее — нижнего. Левое ребро следующей ячейки и нижнее ребро следующей строки обновляются значениями правого и верхнего ребер текущей ячейки. Если текущая строка последняя, обновляется таблица Q. После внутреннего цикла обновляется таблица P. Векторы current и next меняются местами для перехода к следующей строке. Функция возвращает таблицы P и Q.

<h2 id="build">Сборка проекта</h2>

Для сборки проекта необходимо скачать себе на ПК директорию src, затем перейти в неё и собрать проект (для выполнения программы необходимо вызвать ./rsk).
```
cd src
make
./rsk
```
Результаты применения алгоритма можно найти в директории src/results. По умолчанию программа применяет алгоритм к случайной перестановке длины n = 10000, это поведение можно изменить, отредоктировав файл [main.ccp](https://github.com/Xolozop/RSK/blob/42c6863855812f519da593ff51f75d76d8ad9fd1/src/main.cpp#L83).
...
## Практическая часть
### Постановка задачи
Целью данной работы является изучение описанных выше реализаций RSK-алгоритма на эффективность для разных видов входных значений:
1. Упорядоченные множества
2. Равновероятностно случайно сгенерированные множества (реализовано при помощи тасования Фишера-Йетса)
3. Множетва с чередованием элементов по превосходству значения
4. Множества с *k* убывающими наборами упорядоченных по возрастанию элементов
...
|:-----------------------------:|:------------------------:|
|<img src="images/graphs/1-2-a.png">|<img src="images/graphs/1-2-b_fix.png">|

#### 2. Равновероятностно случайно сгенерированный множества
...
#### 3. Множества с чередованием элементов по превосходству значения

* **Возрастающие чередующиеся множества:**
<details>
  <summary><ins>Развернуть/скрыть таблицу результатов</ins></summary>
...
* **Убывающие чередующиеся множества:****
<details>
  <summary><ins>Развернуть/скрыть таблицу результатов</ins></summary>
...
Вида `j*k j*k+1 ... j*k (j-1)*k (j-1)*k+1 ... k 1 ... k-1`
...
Классический RSK алгоритм показал себя как наиболее эффективный на всех тестах. Геометрическая конструкция Вьенно в среднем случае оптимальнее локальных правил на рёбрах (в частности для упорядоченных и частично упорядоченных по возрастанию множеств, подающщихся на вход).
Самую быструю скорость работы классический RSK показал на чередующихся множествах, геометрическая конструкция Вьенно - для случайных и упорядоченных множеств. 

Стоит отметить, что геомерическая конструкция Вьенно хоть и менее эффективна, чем классическая реализация алгоритма, но она позволяет решить такие задачи как поиск максимальной возрастающей подпоследовательности, а также поиск цепей и антицепей.
